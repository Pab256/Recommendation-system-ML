...
def return_book(data):
    ...
    return {'image':image,'book':book_name,'author':author,'rating':rating}
...
with tab1:
    ...
    pop=return_book(data)
    for i in range(len(pop['image'])):
        st.image(pop['image'][i])
        st.write('Book Name:', pop['book'][i])
        st.write('Author:', pop['author'][i])
        st.write('Rating:', str(pop['rating'][i])[:3])
...
with tab2:
    ...
    if text is not None:
        try:
            ...
            book_list=return_book(books_df)
            for i in range(len(book_list['book'])):
                if book_list['book'][i] in result:
                    st.image(book_list['image'][i])
                    st.write('Book Name:', book_list['book'][i])
                    st.write('Author:', book_list['author'][i])
                    st.write('Rating:', str(book_list['rating'][i])[:3])
        except:
            pass
with tab3:
